...
__global__ void mysgemm_v1(int M, int N, int K, float alpha, float *A, float *B,
                           float beta, float *C) {
    int gx = blockIdx.x * blockDim.x + threadIdx.x;  // 全局x      gx是列号
    int gy = blockIdx.y * blockDim.y + threadIdx.y;  // 全局y      gy是当前线程要处理的行号 row_index
    ...
    for (int i = 0; i < K; i++) {
        //A 的形状是M * K
        //M×K（M 行，K 列）
        //gy 是当前线程要处理的行号（row index）
        //i 是列号（col index，循环变量）
    ...
    for (int N: sizes) {
        ...
        try {
            ...
            dim3 blocks((N + threads.x - 1) / threads.x,
                        (N + threads.y - 1) / threads.y);//这是在算grid的大小 threads.x = 一个 block 在 x 方向有多少线程。是 host 端定义的 dim3 对象的成员变量，它只是一个普通的整数
